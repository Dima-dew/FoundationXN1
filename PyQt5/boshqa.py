...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.btn_per.setStyleSheet("background-color: darkGray")
        ...
        self.h0_box.addWidget(self.btn_per)
        #gorizontal 1
        ...
        #vertikal ravshida gorizontallarni qoyib chiqish
        ...
        self.btn_per.clicked.connect(self.foiz)
    
        #ekranga qoshib qoyish
    ...
    def foiz(self):
        ...
        try:
            size = len(expression)
            if expression[size-1] == '+' or expression[size-1] == "-" or expression[size-1] == '/' or expression[size-1] == '*' or expression[size-1] == '%':
                expression = expression[:-1]
            expression = eval(expression)
            expression = int(expression) / 100
            self.label.setText(str(expression))
            self.label2.setText("One percent of given value")
        except SyntaxError:
            self.label2.setText("Error")
